...
const getIcon = (name: string) => {
  ...
  return <Icon className="w-4 h-4 opacity-90" />;
};
...
const TableNode = ({ table, layout }: { table: any, layout: any }) => {
  ...
  return (
    <div
      className={cn(
        "absolute bg-white dark:bg-slate-900 rounded-3xl shadow-xl overflow-hidden flex flex-col pointer-events-auto transition-all duration-300 border-2",
        !isCustomColor && theme.border,
        isDimmed ? "opacity-15 z-20" : isHovered ? "z-40 shadow-2xl scale-[1.02]" : "z-30 hover:scale-[1.01]",
        isHovered && !isCustomColor && !isDragging && "ring-4 ring-primary/30",
        isDragging && "z-50 scale-105 shadow-[0_30px_40px_-10px_rgba(0,0,0,0.2)] cursor-grabbing ring-4 ring-primary/40",
      ...
      style={{
        ...
        boxShadow: (isHovered && isCustomColor && !isDragging) ? `0 0 0 6px ${customColor}40` : undefined
      }}
      data-table={name}
    ...
    >
      <div 
        className={cn("px-4 py-3.5 flex items-center gap-2 select-none border-b border-black/10 dark:border-white/10", !isCustomColor && theme.header, !isCustomColor && theme.text, isCustomColor && "text-white")}
        style={{ backgroundColor: customColor }}
        onMouseDown={handleMouseDown}
      >
        {getIcon(meta.icon || 'table')}
        <span className="font-semibold text-sm tracking-wide truncate">{name}</span>
      </div>
      <div className="flex-1 py-1 bg-white dark:bg-slate-900 z-10 relative">
        {fields.map((f: any) => {
          ...
          if (hoveredEdge) {
            const rel = relationships.find((_, i) => `edge-${i}` === hoveredEdge);
            if (rel && ((rel.sourceTable === name && rel.sourceField === f.name) || (rel.targetTable === name && rel.targetField === f.name))) {
          ...
          return (
            <div 
              ...
              data-field={f.name}
              className={cn(
                "flex justify-between items-center px-3 py-1.5 text-xs transition-colors border-b border-slate-50 last:border-b-0 dark:border-slate-800",
                fieldHighlighted ? "bg-blue-100 dark:bg-blue-900/40" : "hover:bg-slate-50 dark:hover:bg-slate-800"
              )}
            >
              <div className="flex items-center gap-1.5 overflow-hidden">
                {f.isPk ? <LucideIcons.Key className="w-3 h-3 text-yellow-500 shrink-0" /> : <div className="w-3 shrink-0" />}
                <span className={cn("text-slate-700 dark:text-slate-300 truncate", f.isPk && "font-bold")}>{f.name}</span>
              </div>
              <span className="text-slate-400 dark:text-slate-500 font-mono text-[10px] shrink-0">{f.type}</span>
            </div>
          );
        })}
      </div>
...
export const DiagramCanvas = () => {
  ...
  const handleMouseDown = (e: React.MouseEvent) => {
    if (isHandMode || e.target === containerRef.current || (e.target as HTMLElement).closest('svg') || (e.target as HTMLElement).id === 'zoom-layer') {
      setIsPanning(true);
  ...
  return (
    <div 
      ...
      className={cn(
        ...
        isHandMode || isPanning ? "cursor-grab active:cursor-grabbing" : "cursor-default"
      )} 
      onMouseDown={handleMouseDown}
      onWheel={handleWheel}
    >
      ...
      >
        <svg id="svg-layer" className="absolute inset-0 overflow-visible w-full h-full pointer-events-none z-10">
          <defs>
            ...
            <marker id="arrow-end-highlight" markerWidth="14" markerHeight="14" refX="14" refY="7" orient="auto" markerUnits="userSpaceOnUse">
              <path d="M 0 2 L 14 7 L 0 12 z" fill="#3b82f6" />
            </marker>
            <marker id="arrow-start-highlight" markerWidth="14" markerHeight="14" refX="0" refY="7" orient="auto-start-reverse" markerUnits="userSpaceOnUse">
              <path d="M 0 2 L 14 7 L 0 12 z" fill="#3b82f6" />
            </marker>
          </defs>
          <g id="edges-group">
            {relationships.map((rel, i) => {
              ...
              const edgeId = `edge-${i}`;
              const isEdgeHovered = hoveredEdge === edgeId || hoveredTable === rel.sourceTable || hoveredTable === rel.targetTable;
              const isDimmed = (hoveredTable && !isEdgeHovered) || (hoveredEdge && hoveredEdge !== edgeId);

              const markerEnd = (rel.type === '<' || rel.type === '<>') ? (isEdgeHovered ? 'url(#arrow-end-highlight)' : 'url(#arrow-end)') : undefined;
              const markerStart = (rel.type === '<>') ? (isEdgeHovered ? 'url(#arrow-start-highlight)' : 'url(#arrow-start)') : undefined;
